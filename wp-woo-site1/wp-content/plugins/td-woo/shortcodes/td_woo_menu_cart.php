...
class td_woo_menu_cart extends td_block {
    ...
    function render($atts, $content = null) {
        ...
        $menu_cart['cart_subtotal'] = $woocommerce->cart->subtotal;
        if( $menu_cart['cart_subtotal'] == 0 ) {
            $menu_cart['cart_subtotal'] = '0,00 ' . get_woocommerce_currency_symbol();
        } else {
            $menu_cart['cart_subtotal'] .= ' ' . get_woocommerce_currency_symbol();
        }
        ob_start();
        ...
        if ( !$menu_cart['cart_count'] && ( tdc_state::is_live_editor_ajax() || tdc_state::is_live_editor_iframe() ) ) {
            ...
            $menu_cart = array(
                'cart_count' => '3',
                'cart_subtotal' => '200 ' . get_woocommerce_currency_symbol(),
                'cart_contents' => '
        ...
        $cart_normal .= '<a class="tdw-wmc-link" href="' . wc_get_cart_url() . '">';
            ...
            if( $icon != '' ) {
                $cart_normal .= '<div class="tdw-wmc-icon-wrap">';
                    if( base64_encode( base64_decode( $icon ) ) == $icon ) {
                        $cart_normal .= '<span class="tdw-wmc-icon tdw-wmc-icon-svg">' . base64_decode( $icon ) . '</span>';
                    } else {
