...
class td_woo_attribute_filter extends td_block {
	...
	public function get_custom_css() {
		...
		$raw_css = "
			<style>
				...
				.td_woo_attribute_filter .tdw-filter-item:not(.checkbox-tdw-filter-item), 
				...
				.td_woo_attribute_filter .tdw-filter-item:not(.checkbox-tdw-filter-item):hover,
				.td_woo_attribute_filter .tdw-filter-item:not(.checkbox-tdw-filter-item).selected {
		...
		$buffy .= '<div class="' . $this->get_block_classes()  . '" ' . $this->get_block_html_atts() . $data_atts . '>';
			...
			$buffy .= '<div id=' . $this->block_uid . ' class="tdw-block-inner td-fix-index">';
                ...
                $buffy .= '<div class="tdw-filters-wrap">';
                    ...
                    // run trough filters taxonomies to build filters output buffer
                    foreach ( $filters_data['taxonomies'] as $taxonomy ) {
                        ...
                        // run trough taxonomy terms to build terms(filters) output buffer
                        foreach ( $terms as $term ) {
                            ...
                            // filter type terms items html .. <li> tags for color/button & default types / <option> tags for select(dropdown) type
                            switch ( $type ) {
                                case 'color':
    ...
    function get_active_filters( $filters_data, $exclude_current = true ) {
	    ...
	    // check for.. and remove the current queried term from product_cat filter
        if ( $exclude_current ) {
