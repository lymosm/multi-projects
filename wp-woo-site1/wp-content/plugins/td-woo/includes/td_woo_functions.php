...
add_action('tdc_loaded', array('td_woo_config', 'on_tdc_loaded'));
...
function td_woo_on_template_include( $original_template ) {
    ...
    if ( $wp_query->get( 'wc_query' ) && is_tax() && !empty($queried_object) && !td_util::is_mobile_theme() ) {
	    ...
	    if ( function_exists( 'taxonomy_is_product_attribute' ) && taxonomy_is_product_attribute( $queried_object->taxonomy ) ) {
		    ...
		    // read td woo archive attribute(product attribute) template
		    $option_id = 'tdb_woo_archive_attribute_template';

		    // check for individual td woo archive attribute(product attribute) template
...
add_filter( 'admin_init', function (){
	...
	if ( $attribute_taxonomies ) {
		foreach ( $attribute_taxonomies as $tax ) {
			...
			if ( in_array( $product_attr_type, array( 'color' ) ) ) {
...
add_filter( 'woocommerce_add_to_cart_fragments', function( $fragments ) {
	...
	$fragments = array();
	$fragments['div.tdw-wmc-widget-inner'] = '<div class="tdw-wmc-widget-inner" data-cart_contents_count="' . $woocommerce->cart->cart_contents_count . '" data-cart_subtotal="' . $cart_subtotal . '">' . $mini_cart . '</div>';

	return $fragments;
});

add_action( 'woocommerce_product_option_terms', function( $attribute_taxonomy, $i, $attribute ) {
	if ( in_array( $attribute_taxonomy->attribute_type, array( 'color', 'button' ) ) ) {
		?>
