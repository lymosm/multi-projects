...
class Redirection_Admin {
	...
	public function update_nag() {
		...
		// phpcs:ignore
		echo '<div class="update-nag notice notice-warning" style="width: 95%">' . ecs_html($message) . '</div>';
	}
	...
	private function add_help_tab() {
		/* translators: URL */
		$content = sprintf( __( 'You can find full documentation about using Redirection on the <a href="%s" target="_blank">xtoool.com</a> support site.', 'redirecter' ), 'https://www.xtoool.com/wordpress/redirecter/' );
		$title = __( 'Redirecter Support', 'redirecter' );
	...
	public function admin_menu() {
		$hook = add_management_page( 'Redirecter', 'Redirecter', Redirection_Capabilities::get_plugin_access(), basename( REDIRECTION_FILE ), [ $this, 'admin_screen' ] );
	...
	<div class="react-error" style="display: none">
		...
		<p><?php _e( 'Please see the <a href="https://www.xtoool.com/support/problems/">list of common problems</a>.', 'redirection' ); ?></p>
	...
	private function get_current_page( $page = false ) {
		...
		if ( ! $page ) {
			// phpcs:ignore
			$page = isset( $_GET['sub'] ) ? sanitize_text_field($_GET['sub']) : 'redirect';
		}
	...
	public function try_export_rss() {
		...
		if ( isset( $_GET['token'] ) && isset( $_GET['sub'] ) && $_GET['sub'] === 'rss' && Redirection_Capabilities::has_access( Redirection_Capabilities::CAP_REDIRECT_MANAGE ) ) {
			...
			if ( $_GET['token'] === $options['token'] && ! empty( $options['token'] ) ) {
				...
				// phpcs:ignore
				echo esc_url($exporter->get_data( $items, array() ));
				die();
			}
		}
	}
