...
class ProductModel extends Model{
    ...
    public static function getProductListByCate($where, $page, $limit){
        $ret = Db::name('product')
            ...
            ->join('product_img d', 'd.product_id = a.id and d.is_main = 1', 'left')
            ->join('attachment e', 'd.attachment_id = e.id', 'left')
            ->field('a.id, a.name, a.uri, b.short_desc, b.long_desc, b.price, e.uri as img_uri')
            ->where($where)
            ->order('a.id desc')
